      ...
      try {
        if (process.env.DEBUG_WEBHOOKS === "1") {
          console.log("event=sync.fetch_run", {
            ...
            runId: applyAttempt.runId,
            requestId,
            status_before: applyAttempt.status,
          })
        }
        const runJson = await githubRequest<{
          status?: string
          ...
          token,
          key: `gh:run:${request.targetOwner}:${request.targetRepo}:${applyAttempt.runId}`,
          ttlMs: 10_000,
          path: `/repos/${request.targetOwner}/${request.targetRepo}/actions/runs/${applyAttempt.runId}`,
          context: { route: "requests/[requestId]/sync", correlationId: requestId },
        })
        const applyPatch = patchAttemptByRunId(request.runs as RunsState, "apply", applyAttempt.runId, {
          status: runJson.status,
          conclusion: runJson.conclusion ?? undefined,
          completed_at: runJson.completed_at,
          head_sha: runJson.head_sha,
        ...
        if (process.env.DEBUG_WEBHOOKS === "1" && applyPatch) {
          console.log("event=sync.patch_run", {
            kind: "apply",
            runId: applyAttempt.runId,
            old_status: applyAttempt.status,
      ...
      try {
        if (process.env.DEBUG_WEBHOOKS === "1") {
          console.log("event=sync.fetch_run", {
            ...
            runId: destroyAttempt.runId,
            requestId,
            status_before: destroyAttempt.status,
          })
        }
        const runJson = await githubRequest<{
          status?: string
          ...
          token,
          key: `gh:run:${request.targetOwner}:${request.targetRepo}:${destroyAttempt.runId}`,
          ttlMs: 10_000,
          path: `/repos/${request.targetOwner}/${request.targetRepo}/actions/runs/${destroyAttempt.runId}`,
          context: { route: "requests/[requestId]/sync", correlationId: requestId },
        })
        const isDestroy = isDestroyWorkflowRun(runJson.path, env.GITHUB_DESTROY_WORKFLOW_FILE)
        if (isDestroy) {
          const destroyPatch = patchAttemptByRunId(request.runs as RunsState, "destroy", destroyAttempt.runId, {
            status: runJson.status,
            conclusion: runJson.conclusion ?? undefined,
            completed_at: runJson.completed_at,
            head_sha: runJson.head_sha,
          ...
          if (process.env.DEBUG_WEBHOOKS === "1" && destroyPatch) {
            console.log("event=sync.patch_run", {
              kind: "destroy",
              runId: destroyAttempt.runId,
              old_status: destroyAttempt.status,
              new_status: runJson.status,
            })
          }
        ...
        console.warn("[api/requests/sync] destroy run fetch failed for runId:", destroyAttempt.runId, err)
      }
    }
