/**
 * GET /api/environments/:id/drift-latest — Last drift plan run for this environment.
 * Derived from GitHub workflow runs + env drift index (facts-only; no persistent status).
 */
...
export async function GET(
  ...
  for (const run of runs) {
    const indexedEnvId = await getEnvironmentIdByEnvDriftRunId(run.id)
    if (indexedEnvId === environmentId) {
