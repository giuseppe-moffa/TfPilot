...
export type WebhookRouteDeps = {
  ...
  recordDelivery: (deliveryId: string, event: string) => Promise<void>
  getSecret: () => string
  correlatePullRequest: (
...
const realDeps: WebhookRouteDeps = {
  verifySignature,
  hasDelivery,
  recordDelivery,
  getSecret: () => env.GITHUB_WEBHOOK_SECRET,
  correlatePullRequest,
  updateRequest,
  appendStreamEvent,
}
...
export function makeWebhookPOST(deps: WebhookRouteDeps) {
  return async function POST(req: NextRequest) {
    const rawBody = await req.text()
    const signatureHeader = req.headers.get("x-hub-signature-256")
    const secret = deps.getSecret()

    if (!deps.verifySignature(rawBody, signatureHeader, secret)) {
    ...
    if (await deps.hasDelivery(deliveryId)) {
    ...
    if (event === "pull_request") {
      const correlated = await deps.correlatePullRequest(payload as PullRequestCorrelationPayload)
      if (correlated.requestId) {
        try {
          await deps.updateRequest(correlated.requestId, (current) =>
            patchGithubPr(current, payload as PullRequestWebhookPayload)
          )
          await deps.appendStreamEvent({
    ...
    if (!correlated.requestId) {
      ...
      await deps.recordDelivery(deliveryId, event ?? "unknown")
    ...
    try {
      ...
      const [updated, saved] = await deps.updateRequest(correlated.requestId, (current) => {
      ...
      if (
        ...
        const runId = wr.id
        const [updated] = await deps.updateRequest(correlated.requestId, (current) => {
          if (
...
export const POST = makeWebhookPOST(realDeps)
