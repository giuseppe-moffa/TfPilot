...
2. Call `patchRunsAttemptByRunId(current, kind, payload)`: it finds the **attempt** in `request.runs[kind].attempts` whose `runId === workflow_run.id` and updates that attempt’s status, conclusion, completedAt, headSha. **Completion time (completedAt)** is set in `patchAttemptByRunId` (single-source): if the attempt already has completedAt it is kept; else when `status === "completed"` it uses `completed_at ?? updated_at` (the GitHub Actions run API returns `updated_at`, not `completed_at`). If no attempt matches by runId but an attempt exists with matching `head_sha` and no runId, the webhook attaches runId/url to that attempt then patches status/conclusion.
3. Monotonic rules: do not overwrite a completed attempt with in_progress/queued; do not clear conclusion or completedAt. Duplicate status/conclusion for same runId returns no-op so no redundant S3 write.
...
GET `/api/requests/:id/sync` (with or without `?repair=1`) reconciles current attempts. At the start of sync, if `request.lock` exists and its `expiresAt` is in the past, sync clears the lock and persists (so stale locks do not block actions). With `DEBUG_WEBHOOKS=1`, sync logs `event=sync.lock_cleared_expired` when clearing. **Critical rule:** For each kind (plan, apply, destroy), if the **current attempt satisfies needsReconcile** (runId present and either conclusion or completedAt missing), sync fetches that run from the GitHub API (reconcile run fetch uses `bypassCache: true` so completion time is fresh) and patches that attempt via `patchAttemptByRunId`. Completion time is set in `patchAttemptByRunId` from `completed_at ?? updated_at` when status is completed (GitHub run API returns `updated_at`, not `completed_at`). This happens even when `needsRepair(request)` is false and without `?repair=1`, so "stuck destroying" (or planning/applying) and missing completedAt converge when the UI polls sync. A noop cooldown (60s, in-memory) applies when a reconcile fetch returns a non-terminal payload and produces no persisted patch. Sync also runs when `needsRepair(request)` or `?repair=1` for PR/reviews/cleanup and for resolving missing runId (e.g. plan attempt created without runId: sync can resolve runId from the workflow runs list and patch). Sync does not increment currentAttempt or create new attempts; it only refreshes existing attempt records. No canonicalization or legacy repair.
