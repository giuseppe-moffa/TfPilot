...
- **429 backoff:** `hooks/use-request.ts` — when `errorRef.current?.status === 429`, `refreshInterval` returns `SYNC_INTERVAL_RATE_LIMIT_BACKOFF_MS` (line 109); `onErrorRetry` for 429 uses `setTimeout(revalidate, SYNC_INTERVAL_RATE_LIMIT_BACKOFF_MS)` (lines 127–129).
